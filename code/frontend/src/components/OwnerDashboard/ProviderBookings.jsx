...
const ProviderBookings = () => {
    ...
    const fetchListings = async () => {
        try {
            ...
            if (user?.role === 'hotel_owner') {
                const res = await authFetch('http://localhost:3001/api/hotels/provider');
                if (res.ok) hotels = (await res.json()).response || [];
            }
            if (user?.role === 'vehicle_owner') {
                const res = await authFetch('http://localhost:3001/api/vehicles/owner');
                if (res.ok) vehicles = await res.json() || [];
            }
            if (user?.role === 'tour_guide') {
                const res = await fetch(`http://localhost:3001/api/tour-guides/user/${user._id}`);
                if (res.ok) {
                    const data = await res.json();
    ...
    const handleUpdateStatus = async (id, status) => {
        try {
            ...
            const res = await authFetch(`http://localhost:3001/api/bookings/${id}/${endpoint}`, {
                method: 'PUT',
