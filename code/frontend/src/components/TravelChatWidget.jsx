...
const TravelChatWidget = ({ onSendMessage, user }) => {
  ...
  const totalPrice = activeBookingHotel ? nights * activeBookingHotel.pricePerNight * rooms : 0;

  const handleOpenBooking = (hotel) => {
    if (!user) {
      alert("Please sign in first to make a reservation.");
      setIsOpen(false);
      navigate("/login");
      return;
    }
    ...
    setEndDate(getDayAfterTomorrowString());
    setRooms(1);
    setGuests(1);
  };
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/bookings', {
        ...
        body: JSON.stringify({
          userId: user._id,
          hotelId: activeBookingHotel._id,
          startDate,
          endDate,
      ...
      if (response.ok) {
        ...
        const successMessage = {
          role: 'bot',
          content: `🎉 **Booking Confirmed Successfully!**\n\nI have registered your booking for **${activeBookingHotel.name}** in *${activeBookingHotel.location}*.\n• **Dates**: ${startDate} to ${endDate}\n• **Rooms**: ${rooms} | **Guests**: ${guests}\n• **Total Paid**: **$${totalPrice}**\n\nEnjoy your stay in Sri Lanka! You can view this reservation in your [My Bookings Dashboard](/my-bookings).`,
          timestamp: new Date()
        };
              ...
              <div className="p-2 flex-1 flex flex-col justify-between">
                ...
                <div className="flex gap-1 mt-2.5">
                  <button
                    ...
                    className="flex-1 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded text-[9px] font-semibold transition-colors cursor-pointer border-none"
                  >
                    Details
                  </button>
                  <button
                    onClick={() => handleOpenBooking(hotel)}
                    className="flex-1 py-1.5 bg-orange-500 hover:bg-orange-600 text-white rounded text-[9px] font-semibold transition-colors cursor-pointer border-none"
                  >
                    Book Now
                  </button>
                </div>
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans">
      {isOpen ? (
        <div className="w-80 sm:w-[26rem] h-[34rem] flex flex-col rounded-2xl shadow-2xl overflow-hidden border border-gray-150/70 relative" style={{ backgroundColor: '#FDFBEE' }}>
          
          {/* Direct Booking Panel Overlay */}
          {activeBookingHotel && (
            <div className="absolute inset-0 bg-white/95 backdrop-blur-xs z-30 flex flex-col p-5 font-sans">
              <div className="flex items-center justify-between pb-3 border-b border-gray-100">
                <div>
                  <h3 className="font-bold text-sm text-gray-800 m-0">In-Chat Booking</h3>
                  <p className="text-xs text-gray-500 m-0">{activeBookingHotel.name}</p>
                </div>
                <button 
              ...
              <div className="flex-1 overflow-y-auto py-4 space-y-4 text-sm text-gray-700">
                <div className="bg-orange-50/50 border border-orange-100 p-3 rounded-xl flex items-center justify-between">
                  ...
                  <span className="font-bold text-orange-600 text-base">${activeBookingHotel.pricePerNight}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 text-xs focus:outline-none focus:ring-1 focus:ring-orange-500"
                    />
                  </div>
                  <div>
                ...
                {nights > 0 ? (
                  <div className="bg-gray-50 border border-gray-150 p-3.5 rounded-xl space-y-2">
                    ...
                    <div className="flex justify-between text-sm pt-2 border-t border-gray-200 font-semibold text-gray-800">
                      <span>Total Amount:</span>
                      <span className="text-orange-600 font-bold">${totalPrice}</span>
                    </div>
                  </div>
                ) : (
                  <div className="bg-red-50 text-red-500 p-3 rounded-lg text-xs text-center font-medium border border-red-100">
              ...
              <div className="pt-3 border-t border-gray-100 flex gap-3">
                <button 
                  ...
                  className="flex-1 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-full font-semibold transition-colors cursor-pointer border-none text-xs disabled:opacity-50"
                >
                  Cancel
                </button>
                <button 
                  onClick={handleConfirmBooking} 
                  disabled={nights <= 0 || isBookingLoading}
                  className="flex-1 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-full font-bold transition-colors cursor-pointer border-none text-xs flex items-center justify-center gap-1 disabled:opacity-50"
                >
