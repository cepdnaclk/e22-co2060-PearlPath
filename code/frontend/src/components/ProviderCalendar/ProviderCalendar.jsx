...
const ProviderCalendar = ({ serviceType, serviceId, title }) => {
    ...
    const fetchAvailability = async () => {
        try {
            setLoading(true);
            const res = await fetch(`http://localhost:3001/api/${serviceType}/${serviceId}/availability`);
            if (res.ok) {
    ...
    const handleDateClick = async (value) => {
        ...
        try {
            const res = await authFetch(`http://localhost:3001/api/${serviceType}/${serviceId}/manage-availability`, {
                method: 'POST',
