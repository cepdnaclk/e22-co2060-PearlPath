...
const AdminDashboard = () => {
    ...
    const fetchPayments = async () => {
        ...
        setPayments([]);
        try {
            const res = await authFetch('http://localhost:3001/api/payments/admin?status=submitted');
            if (res.ok) {
    ...
    const handlePaymentAction = async (id, action) => {
        try {
            ...
            const res = await authFetch(`http://localhost:3001/api/payments/admin/${id}/${action}`, {
                method: 'PUT',
    ...
    const fetchStats = async () => {
        ...
        setError(null);
        try {
            const statsRes = await authFetch('http://localhost:3001/api/admin/stats');
            if (statsRes.ok) {
    ...
    const fetchRoleData = async (role) => {
        ...
        setRoleData([]);
        try {
            const res = await authFetch(`http://localhost:3001/api/admin/roles/${role}`);
            if (res.ok) {
    ...
    const handleUserAction = async (id, status) => {
        try {
            const res = await authFetch(`http://localhost:3001/api/admin/users/${id}/status`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleListingAction = async (type, id, status, userId) => {
        try {
            const res = await authFetch(`http://localhost:3001/api/admin/listings/${type}/${id}/status`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ status })
            });
    ...
    const handleDeleteUser = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await authFetch(`http://localhost:3001/api/admin/users/${id}`, { method: 'DELETE' });
                    if (res.ok) {
    ...
    const handleEditUser = (user) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await authFetch(`http://localhost:3001/api/admin/users/${user._id}`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                                    ...
                                    {payments.map(payment => (
                                        <div key={payment._id} className="flex flex-col sm:flex-row justify-between p-5 border border-gray-100 rounded-xl bg-gray-50">
                                            ...
                                            <div className="flex flex-wrap gap-3 mt-4 sm:mt-0 justify-end items-center">
                                                <a href={`http://localhost:3001${payment.bankSlipUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-blue-50 text-blue-600 px-4 py-2 rounded-lg font-bold">
