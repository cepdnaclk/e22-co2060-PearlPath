...
const Experiences = () => {
  ...
  const fetchExperiences = async (currentPage) => {
    ...
    try {
      let url = new URL('http://localhost:3001/api/experiences');
  ...
  const handleDeleteClick = async (experience) => {
    ...
    try {
      const response = await authFetch(`http://localhost:3001/api/experiences/${experience._id}`, {
        method: 'DELETE'
      });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (editingExperienceId) {
        response = await authFetch(`http://localhost:3001/api/experiences/${editingExperienceId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(submissionData)
        });
      } else {
        response = await authFetch('http://localhost:3001/api/experiences', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(submissionData)
        });
